...
export async function scheduleEventReminders(events: Item[]) {
  try {
    ...
    console.log(
      `Scheduled notifications for ${todayEvents.length} today and ${tomorrowEvents.length} tomorrow events`
    );
  } catch (error) {
    console.log(
      "Notification scheduling failed (this is expected in Expo Go):",
      error
    );
  }
}
